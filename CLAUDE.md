...
After any code change, run `make check` before considering the task done. It runs `./test.sh` (tests + 95% coverage floor), `gofmt -l`, and `go vet`. CI runs the same target on push and PR, so a failing `make check` will fail CI.
